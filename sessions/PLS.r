...
ggsave("pls_graph_1.png", plot="pls_graph_1", width = 6, height =4, dpi = 300)
...
ggsave("pls_graph_2.png", plot = "pls_graph_2" width = 6, height =4, dpi = 300)
...
# Save it
ggsave("pcr_1.png", plot = "results", width = 6, height = 4, dpi = 300)
...
ggplot(results_2, aes(x = Observed, y = Predicted)) +
  ...
  theme_minimal(base_size = 14)
# Save it
ggsave("pcr_2.png", plot = "results_2" width = 6, height = 4, dpi = 300)

#Linear Regression for Ferriting and Free Iron against Anemia Risk 
Ferritin_Regression <- ggplot(dat,aes(MBX_Ferritin, AnemiaRiskScore)) + 
stat_summary(fun.data=mean_cl_normal) + 
  geom_smooth(method='lm', formula= y~x)
ggsave("Ferritin_Regression.png", plot = Ferritin_Regression, width = 6, height = 4, dpi = 300)

Iron_Regression <- ggplot(dat,aes(MBX_FreeIron, AnemiaRiskScore)) +
stat_summary(fun.data=mean_cl_normal) + 
  geom_smooth(method='lm', formula= y~x)
ggsave("Iron_Regression.png", plot = Iron_Regression, width = 6, height = 4, dpi = 300)
